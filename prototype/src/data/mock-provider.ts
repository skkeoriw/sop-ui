...
const mockNodeDraftSchema: NodeDraftSchema = {
  ...
  fields: [
    ...
    { name: "entry_input_name", label: "Entry input name", type: "slug", required: false, default: "prompt", mapsTo: "entry_inputs" },
    { name: "input_type", label: "Entry input type", type: "enum", required: false, default: "string", mapsTo: "entry_inputs.*.type" },
    { name: "input_value_type", label: "Entry input value type", type: "enum", required: false, default: "text", mapsTo: "entry_inputs.*.value_type" },
    { name: "output_name", label: "Output name", type: "slug", required: false, default: "artifact", mapsTo: "outputs" },
    { name: "output_path", label: "Output path", type: "path_pattern", required: false, default: "raw/node-runs/{run_id}/outputs/outputs/{output_name}.json", mapsTo: "outputs.*.path" },
  ],
  ...
  async createNodeDraft(target, _instanceId, input: NodeDraftInput): Promise<NodeDraft> {
    ...
    const draft: NodeDraft = {
      ...
      node: input.node_draft || {
        ...
        entry_inputs: { [input.entry_input_name || input.input_name || "prompt"]: { kind: "scalar", type: input.input_type || "string", value_type: input.input_value_type || "text", required: true } },
        handoff: {
          accepts: [{ role: "upstream_outputs_dir", required: false }, { role: "instruction", required: true }],
          produces: { outputs_dir: "raw/node-runs/{run_id}/outputs", manifest: "raw/node-runs/{run_id}/outputs/manifest.json" },
        },
        inputs: { [input.entry_input_name || input.input_name || "prompt"]: { kind: "scalar", type: input.input_type || "string", value_type: input.input_value_type || "text", required: true } },
        ...
        items: [
          {
            ...
            path: "raw/node-runs/{source_node_run_id}/outputs/outputs/source-url.txt",
          },
        ],
      },
      relay_package: {
