...
function mapNodeRunRelayItem(raw: Record<string, unknown>) {
  ...
  return {
    ...
    valuePreview: raw.value_preview ? String(raw.value_preview) : raw.valuePreview ? String(raw.valuePreview) : undefined,
    artifact,
  };
}
...
function mapNodeConfig(raw: Record<string, unknown>, nodeId: string): NodeConfig {
  ...
  return {
    ...
    entryInputs: ((raw.entry_inputs || raw.entryInputs) as Record<string, unknown>) || {},
    handoff: (raw.handoff as Record<string, unknown>) || {},
    workflowInputs: ((raw.workflow_inputs || raw.workflowInputs) as Record<string, unknown>) || {},
...
export const sopProvider: SopDataProvider = {
  ...
  async getWorkflowEdgeEvaluation(runtime, instanceId, workflowId, evaluationId): Promise<WorkflowEdgeResult> {
    return requestJson<Record<string, unknown>>(
      `${runtime.endpoint}/api/sop/${encodeURIComponent(instanceId)}/workflows/${encodeURIComponent(workflowId)}/edges/evaluations/${encodeURIComponent(evaluationId)}`
    );
  },
  ...
  async generateWorkflowEdgeRuntimeSop(runtime, instanceId, workflowId, input: WorkflowEdgeRequest): Promise<WorkflowEdgeResult> {
    return postJsonResult<Record<string, unknown>>(
      `${runtime.endpoint}/api/sop/${encodeURIComponent(instanceId)}/workflows/${encodeURIComponent(workflowId)}/edges/runtime-sop`,
      input
    );
  },
