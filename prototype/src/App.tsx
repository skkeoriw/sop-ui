...
function buildEdgeDraftApplyScript(input: {
  ...
  return [
    ...
    "  case \"$code\" in",
    `    ${exitCancel}) echo "  - 用户取消了执行。可重新运行：bash /tmp/apply-edge.sh --auto-confirm" ;;`,
    `    ${exitPreflight}) echo "  - 前置检查失败，建议先做 dry-run 检查上下文" ;;`,
    `    ${exitCommit}) echo "  - commit 失败，请先修复后重跑" ;;`,
    `    ${exitPush}) echo "  - push 失败，请修复凭据后重跑" ;;`,
    "    *) echo \"  - 查看上方日志中的详细提示。\" ;;",
    "  esac",
    "  echo \"exit code: $code\"",
    "  exit \"$code\"",
